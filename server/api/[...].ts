...
io.on("connection", async (socket) => {
	...
	socket.on("musicId", async (musicId) => {
		if (socket.handshake.headers.origin != null) {
			...
			const index = listMusicClient.findIndex((item: any) => item.socket_ip === getSocketRooms(socket.id))
			...
			if (index !== -1 && listMusicClient[index].musicId === musicId) {
				...
				listMusicClient.splice(index, 1)
				io.to("server").emit("success", "Music successfully changed to " + title)
			}
	...
	socket.on("sendFile", (data) => {
		...
		data.selectedItems.forEach(async (item: any) => {
			item.announcement_in_list.forEach(async (ann: any) => {
				...
				const fullPath = path.join(process.cwd(), fileUrl.replace("\\_nuxt", ""))
	...
	socket.on("disconnect", () => {
		const index = listSocket.findIndex((item: any) => item.socket_id === socket.id)
		if (index !== -1) {
			console.log(`User disconnected : {IP = ${getSocketRooms(socket.id)}, ID = ${socket.id}}`)
