<template  @drop.prevent="dropHandler" @dragover.prevent>
    ...
    <canvas ref="canvasRef" class="rounded-md" @drop.prevent="handleDrop" @dragover.prevent="handleDragOver"></canvas>
    <div class="flex items-center rounded-md space-x-3 bg-[#006780] p-2"
        :style="{ height: `calc(100vh - ${canvasHeight}px)` }">
        <button @click="addText" class="border-2 border-Primary bg-PrimaryContainer p-1 rounded-lg h-[55px] w-[55px]">
            <img src="\AddText.png" class="w-[40px] h-[40px]" alt="">
        </button>
        <button @click="addRect" class="border-2 border-Primary bg-PrimaryContainer p-1 rounded-lg h-[55px] w-[55px]">
            <img src="\AddText.png" class="w-[40px] h-[40px]" alt="">
        </button>
        ...
        <select v-model="fontWeight" v-on:change="fontWeightOnChange"
            ...
            <option value="normal" class="text-OnPrimaryContainer">Normal</option>
            <option value="bold" class="text-OnPrimaryContainer">Bold</option>
        </select>
        <div
            ...
            <input type="color" class="h-[25px] w-full">
        </div>
        <div
            class="flex flex-col items-center border-2 border-Primary bg-PrimaryContainer text-OnPrimaryContainer rounded-lg h-[55px] p-1 w-[100px]">
            <div class="text-OnPrimaryContainer">Font Color</div>
            <input type="color" class="h-[45px] w-full">
        </div>
