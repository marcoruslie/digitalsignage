<template>
    ...
    <div class="bg-cyan-700 h-svh w-full p-2 flex space-x-3">
        ...
        <div class="h-full w-[80%] bg-white rounded-xl p-2 flex-col flex">
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <button @click="toggleModal(modalShowPengumuman)"
                        class="bg-Primary rounded text-OnPrimary px-4 py-1 hover:text-OnPrimaryContainer hover:bg-PrimaryContainer hover:duration-300">List
                        Pengumuman</button>
                    <input type="text" v-model="imageUrl" class="border-gray-400 border-2 rounded"
                        placeholder="URL FILE" @focusout="onFileChange" />
            ...
            <div v-else-if="uploadType == 'ekstraksi'" class="flex-grow flex p-1 space-x-1 justify-end overflow-hidden">
                <div class="w-[10%] max-h-full h-full flex-col flex items-center overflow-auto">
                    <div class="text-Primary font-bold text-lg">Property</div>
                    
                </div>
                <div class="w-[90%] max-h-full h-full flex-col flex items-center overflow-hidden duration-1000"
                    :class="!minimize ? 'max-w-[45%]' : 'max-w-full'">
                    <div class="text-Primary font-bold text-lg">HASIL EKSTRAKSI KONTEN</div>
                    <canvas class="text-base border-2 border-black rounded flex flex-grow w-full" ref="canvasRef"></canvas>
                </div>
                <img :src="minimize == false ? '/icon-minimize.png' : '/icon-maximize.png'" class="cursor-pointer w-10 h-10"
                    @click="minimize = !minimize">
                <div class="w-[45%] max-h-full h-full flex-col flex items-center overflow-hidden duration-1000"
                    :class="!minimize ? 'max-w-full' : 'max-w-0'">
                    <div class="text-Primary font-bold text-lg">KONTEN POSTER/GAMBAR</div>
                    <div v-show="loading" class="w-full h-6 bg-white text-center">
                        <div class="absolute text-center font-bold">{{ loadingProgress }}%</div>
                        <div class="h-full bg-green-600 rounded-md" :style="{ width: loadingProgress + '%' }">
                        </div>
                    </div>
                    <div class="flex flex-col h-full w-full relative border-2 border-black rounded">
                        <div class="relative w-full h-full overflow-hidden" v-if="isImageLoaded" ref="containerRef">
                            <img :src="imageUrl" alt="" class="absolute top-0 left-0 h-full w-full">
                            <div v-for="(word, index) in detectedWords" :key="index"
                                class="absolute text-center bg-transparent rounded"
                                :style="{ left: word.bbox.left + '%', top: word.bbox.top + '%', width: word.bbox.width + '%', height: word.bbox.height + '%', backgroundColor: word.selected ? 'blue' : 'gray', fontSize: word.fontSize <= 10 ? '10px' : word.fontSize + 'px' }">
                                {{ word.text }}
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center space-x-3 w-full">
                            <div class="flex-col w-full p-1">
                                ...
                                <div class="flex justify-between w-full items-center">
                                    ...
                                    <div class="w-full">
                                        <div>Tanggal : {{ new Date(data.createdAt).toLocaleDateString('en-GB', {
                        day:
                            '2-digit', month: 'long', year: 'numeric'
                    }) }}</div>
                                        <div>Jam : {{ new Date(data.createdAt).toLocaleTimeString('en-US', {
                        hour:
